...
def pre_process_actions(delta_pose: torch.Tensor, gripper_command: bool) -> torch.Tensor:
    ...
    if "Reach" in args_cli.task or "Position-Calibration" in args_cli.task or "Pose-Calibration" in args_cli.task or "Rotation-Calibration" in args_cli.task:
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # apply actions
            # if "Position-Calibration" in args_cli.task:
            #     # for position calibration, we need to set the gripper command to 0
            #     actions  = actions[:, 0:3]
            obs, reward, reset, time_outs, extras = env.step(actions)
            print("Reward: ", reward)
